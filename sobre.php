...
$stmt = $conn->prepare("SELECT contenido FROM paginas_estaticas WHERE id = ?");
$stmt->bind_param("i", $id);
$stmt->execute();
$stmt->bind_result($contenido_sobre);
$stmt->fetch();
$stmt->close();
?>
...
<div class="subnav no-leer" style="max-width:100% !important; display:flex; flex-wrap:wrap; align-items:center; gap:10px;">
  ...
  <div class="pg-cntr-987">
    <?php echo html_entity_decode($contenido_sobre, ENT_QUOTES, 'UTF-8'); ?>
</div>
</div>
